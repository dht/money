...
export const budgetsByPeriod = createSelector(
    ...
    (budgets, plannedBudget, income) => {
        ...
        return keys.reduce((output, periodId) => {
            ...
            categories.forEach(catKey => {
                ...
                output[periodId][catKey].actualPercent = Math.abs(
                    Math.floor((100 * output[periodId][catKey].actual) / income)
                );
            });
            ...
            categories.forEach(catKey => {
                ...
                output[periodId][catKey].plannedPercent = Math.abs(
                    Math.floor(
                        (100 * output[periodId][catKey].planned) / income
                    )
                );
            });
...
export const sumByPeriod = createSelector(
    ...
    (budgets, income) => {
        ...
        return keys.reduce((output, periodId) => {
            ...
            if (income) {
                output[periodId].actualPercent = Math.floor(
                    (100 * output[periodId].actual) / income
                );
                output[periodId].plannedPercent = Math.floor(
                    (100 * output[periodId].planned) / income
                );
            }
...
export const calcBudgetSelector = createSelector(
    ...
    (lines, names) => {
        ...
        return keys.reduce((output, key) => {
            ...
            output[month][category.categoryId] =
                output[month][category.categoryId] || {};
            output[month][category.categoryId].title = category.categoryTitle;
            output[month][category.categoryId].actual =
                output[month][category.categoryId].actual || 0;
