...
export const addCategoryName = (id, data) => {
    const ref = categoriesRef.child(id).child('names').push(),
        key = ref.key;
...
export const createBoard = (boardId, userId, mode) => {
    return createPermissions(boardId, userId)
        .then(() => {
...
export const removeCategoryName = (id, nameId) => {
    const ref = categoriesRef.child(id).child('names').child(nameId);
    return ref.remove();
}
...
export const savePlannedBudget = (period, categoryId, value) => {
    return plannedBudgetsRef.child(period).child(categoryId).set(value);
}
...
export const setProjectLine = (id, week, value) => {
    const ref = projectsRef.child(id).child('lines').child(week);
