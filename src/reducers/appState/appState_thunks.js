...
import {
    currentIndexSelector, currentWeekSelector, showListSelector,
    showPageSelector
...
export const updateItem = (id, data, immediate, noChangeLog) => {
    return (dispatch) => {
        ...
        api.updateData(id, data, noChangeLog)
            .then((response) => {
                if (!immediate) {
                    dispatch(actions.changeLine(id, data))
                }
                if (!noChangeLog) {
                    ...
                    dispatch(actions.addChangeLog(id, {...data, id: keyChangeLog}))
                }
            })
    }
}
