...
export class New extends Component {
    ...
    clear = () => {
        ...
        setTimeout(() => document.querySelector('.react-autosuggest__input').focus(), 0);
    }
    ...
    render() {
        ...
        return (
            <div className="New-container">
                {/*<Input placeholder="תאריך" onChange={(date) => this.setState({date})} value={date} id="date"*/}
                ...
                <Autocomplete
                    ...
                    id="first"
                />
                <Input placeholder="סכום" onChange={(sum) => this.setState({sum})} value={sum}
                       className="sum" style={{width: 96, direction: 'ltr', textAlign: 'right'}} onEnter={this.save}/>
                <CashCredit isCredit={isCredit} onToggle={() => this.setState({isCredit: !isCredit})}/>
